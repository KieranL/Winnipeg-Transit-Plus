...
object FavouriteStopsList {
    ...
    fun loadFavourites(): Boolean {
        ...
        if (isLoadNeeded) {
            try {
                ...
                Logger.getLogger().error(ex, "Error loading favourite stops")
                isLoadNeeded = true
            }
    ...
    private fun getValue(tag: String, originalNode: Node): String? {
        return try {
            ...
            Logger.getLogger().error(ex, "Error getting favourite node value")
            null
        }
    ...
    private fun getXML(inputStream: InputStream): LoadResult<Document> {
        return try {
            ...
            Logger.getLogger().error(ex, "Error loading favourite stop XML file")
